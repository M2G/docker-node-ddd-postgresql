...
describe('Routes: GET cityEntity', () => {
  ...
  let cityId1: number | any;

  let cityId2: number | any;
  ...
  describe('Should return City', () => {
    beforeEach((done) => {
      cityRepository
        ...
        .then(() => cityRepository.create({ ...CITY_1 }))
        .then((res: any) => {
          cityId1 = res.city_id;
        }).then(() => cityRepository.create({ ...CITY_2 }))
        .then((res: any) => {
        cityId2 = res.city_id;
        done();
      })
